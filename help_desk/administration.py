...
class XLSXImporter:
    ...
    def parseSutartys(self, sheet):
        for i in range(sheet.nrows):
            ...
            client = Client.get(id=self.trimID(sheet.cell(i, 4), "K"))
    ...
    def parseKreipiniai(self, sheet):
        for i in range(sheet.nrows):
            ...
            issue = Issue(id=id, client=client, service=service, type=type, receive_type=receive_type, title=title, description=description, created=created, closed=closed, status=status,
                          rating=rating, current=current, previous=previous)
            issue.save()
